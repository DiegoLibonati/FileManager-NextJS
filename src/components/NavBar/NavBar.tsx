  ...
  return (
    <nav className="flex relative flex-row items-center justify-around w-full h-full lg:flex-col lg:justify-start">
      <Link href={"/"} aria-label="go to home page" className="lg:my-4">
        {pathname !== "/" ? <AiOutlineHome fontSize={24}></AiOutlineHome> : null}
        {pathname === "/" ? <AiFillHome fontSize={24} fill={"#8357fe"}></AiFillHome> : null}
      </Link>
      <Link href={"/folders"} aria-label="go to folders page" className="lg:my-4">
        {pathname !== "/folders" ? <AiOutlineFolder fontSize={24}></AiOutlineFolder> : null}
        {pathname === "/folders" ? (
          <AiFillFolder fontSize={24} fill={"#8357fe"}></AiFillFolder>
        ) : null}
      </Link>
      <Link href={"/cloud"} aria-label="go to cloud page" className="lg:my-4">
        {pathname !== "/cloud" ? <AiOutlineCloud fontSize={24}></AiOutlineCloud> : null}
        {pathname === "/cloud" ? <AiFillCloud fontSize={24} fill={"#8357fe"}></AiFillCloud> : null}
      </Link>
      <Link href={`/folder/root`} aria-label="go to folder root page" className="lg:my-4">
        {!pathname.includes("/folder/") ? <VscRootFolder fontSize={24}></VscRootFolder> : null}
